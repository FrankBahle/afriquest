...
function AdminGlobalSettingsScreen() {
  ...
  return <div style={styles.panel}>
    ...
    <div style={{ ...styles.smallCard, marginTop: 18 }}><form onSubmit={submit} style={formGrid}>
      ...
      <Field label="Default language"><select style={inputStyle} value={form.defaultLanguage || 'en'} onChange={(e)=>setForm({...form,defaultLanguage:e.target.value})}><option value="en">English</option><option value="fr">French</option><option value="zu">isiZulu</option></select></Field>
      <Field label="App theme"><input style={inputStyle} value={form.appTheme || ''} onChange={(e)=>setForm({...form,appTheme:e.target.value})} /></Field>
      <label><input type="checkbox" checked={!!form.cardFlipEnabledByDefault} onChange={(e)=>setForm({...form,cardFlipEnabledByDefault:e.target.checked})} /> Card flip enabled by default</label>
